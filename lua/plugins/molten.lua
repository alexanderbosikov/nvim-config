...
-- Код-ячейка — только ```python: фенсы с другим языком — это либо примеры кода
-- внутри markdown-ячейки, либо магика языка (```sql), у которой в теле нет строки
-- `%%sql` — ядру такое отдавать нельзя, будет SyntaxError. Значит sql-ячейки molten
-- не выполняет: их разбирает jupyter.nvim, который собирает магику из info-строки.
