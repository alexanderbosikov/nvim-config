...
- **Причина:** не баг конфига, а формат jupytext. В markdown-представлении магика языка выносится из тела ячейки в info-строку фенса, аргументы — в cell metadata `magic_args`. Molten отдаёт ядру тело фенса, а магики там уже нет. В самом `.ipynb` при этом всё цело — ломается только выполнение в буфере.
- **Фикс (в конфиге):** `lua/custom/ipynb_magics.lua` при чтении возвращает ячейку к виду из `.ipynb` (` ```python ` + `%%sql ...` первой строкой); обратное преобразование не нужно — при записи jupytext снова выносит язык в info-строку. Плюс `queries/markdown/injections.scm` (полная замена рантайм-запроса, поэтому НЕ в `after/`): в такие фенсы инъектируется sql, а не python — иначе otter отдаёт SQL-тело pyright и получаем диагностику на каждой sql-ячейке.
- **Проверка:** открыть ноутбук с `%%sql`-ячейкой → в буфере ` ```python ` и `%%sql`, подсветка SQL, `gc` комментирует как `-- `; `<leader>jc` выполняется в ядре.
- **Границы:** нормализация только для `%%sql` (нужен другой язык магики — добавить в `MAGIC_LANGS` и пару паттернов в запросы). Фенсы внутри `<!-- #region -->` — это код в markdown-ячейке, они не трогаются и не выполняются.
